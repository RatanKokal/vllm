...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        self.num_steps: int = envs.VLLM_MULTI_STEP_DECODE_N
        valid_num_steps = (1, 2, 4, 8)
        if self.num_steps not in valid_num_steps:
            raise ValueError(
                f"VLLM_MULTI_STEP_DECODE_N={self.num_steps} is not supported."
            )
        if self.num_steps > 1:
            if self.scheduler_config.max_model_len % self.num_steps != 0:
                raise ValueError(
                    f"max_model_len ({self.scheduler_config.max_model_len}) "
                    f"must be divisible by VLLM_MULTI_STEP_DECODE_N ({self.num_steps})."
                )
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            with record_function_or_nullcontext("schedule: allocate_slots"):
                while True:
                    # For pure decode steps pre-allocate (num_steps - 1)
    ...
    def _update_request_as_session(
        ...
        """
        # 1. Keep ONLY computed output tokens. Discard the final sampled 
        # output token and any speculative placeholders.
        num_computed_tokens = session.num_computed_tokens
        kept_output_tokens = session._all_token_ids[
            session.num_prompt_tokens : num_computed_tokens
        ]
        
        # 2. Hard truncate the master token list
        del session._all_token_ids[num_computed_tokens:]
        session._output_token_ids.clear()
        assert session.prompt_token_ids is not None
        
        # 3. Promote the kept output tokens into the prompt
        session.prompt_token_ids.extend(kept_output_tokens)

        # 4. Handle multi-modal positional offsets (if applicable)
        ...
        # 5. Append the NEW streaming input tokens
        session._all_token_ids.extend(update.prompt_token_ids or ())
        session.prompt_token_ids.extend(update.prompt_token_ids or ())
        
        # 6. CRITICAL: Update block hashes so Prefix Caching recognizes the new state
        if hasattr(session, 'get_hash_new_full_blocks') and session.get_hash_new_full_blocks is not None:
            session.block_hashes.extend(session.get_hash_new_full_blocks())
        elif hasattr(session, 'update_block_hashes'):
            session.update_block_hashes()

        # 7. Reset state trackers
        ...
        # Push back to WAITING to be picked up by the next wave
